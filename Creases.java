...
private void makeDirections() {
	...
	System.out.println("ok, "+corners.size()+"; "+activeCorners.size()+": "+directions.size());
}
private void makeBisectors() {
	while(activeCorners.size()>0) {
		...
		System.out.println("ok,now here "+corners.size()+"; "+activeCorners.size()+": "+directions.size());
		...
		for(int i=0;i<activeCorners.size();i++) {
			...
			for(int j=0;j<activeCorners.size();j++) {
				...
				if(i!=j&&p1.x==p2.x&&p1.y==p2.y) {
					System.out.println(i+","+j+"removing");
